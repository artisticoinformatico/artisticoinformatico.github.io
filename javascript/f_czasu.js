function nazwa_miesiaca(nrm){
    ...
    if(nrm==8) return "WrzeĹnia";
    if(nrm==9) return "PaĹşdziernika";
    if(nrm==10) return "Listopada";
    if(nrm==11) return "Grudnia";
}
function dzien(dzn){
    if(dzn==0) return "Niedziela";
    if(dzn==1) return "PoniedziaĹek";
    if(dzn==2) return "Wtorek";
    if(dzn==3) return "Ĺroda";
    if(dzn==4) return "Czwartek";
    if(dzn==5) return "PiÄtek";
...
function DataOdDo(data){
    ...
    function pisz_data(){
        ...
        let wypis=("<h2>"+"DziĹ jest: "+dzien(data.getDay())+(" ")+data.getDate()+(" ")+
        ...
        wypis+='<br> Data z pola: ';
            ...
            wypis+=dzien(data_zpola.getDay())+' '+data_zpola.getDate()+' '+nazwa_miesiaca(data_zpola.getMonth())+ ' '+data_zpola.getFullYear()+'</h2>';
